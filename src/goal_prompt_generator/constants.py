...
GOAL_RUNTIME_CONTINUATION_CONTRACT = (
    "When this contract is executed through Hermes `/goal`, shape every intermediate assistant final response so "
    "the GoalManager judge has an unambiguous reason to continue: continue autonomously until all acceptance criteria "
    "validate or Hermes goal max_turns pauses the loop; do not ask the user for input, guidance, confirmation, or "
    "permission; do not state that the overall goal is complete, blocked, unachievable, waiting for input, or needing "
    "human action unless every acceptance criterion and validation requirement is satisfied or a non-bypassable legal, "
    "payment, credential, or safety requirement prevents every remaining autonomous path. For every non-final turn, "
    "end the response with `GOAL_RUNTIME_STATUS: CONTINUE — next autonomous step queued` and name the next YAML task id. "
    "For the final validated turn only, end with `GOAL_RUNTIME_STATUS: COMPLETE — all acceptance criteria and validations satisfied`. "
    "If a task encounters friction, record it in the YAML task's mutable `learnings` or `gotchas`, choose the safest remaining "
    "autonomous alternative, and keep working rather than pausing for human input."
)
...
CLAUDE_CLI_EXECUTION_CONTRACT = (
    ...
    "with the full required flag set: `--print`, `--add-dir <list-of-directories>`, "
    ...
    "`--mcp-config <mcp-json-file>`, `--settings <settings-json-file>`, `--strict-mcp-config`, "
    "`--system-prompt-file <file>`, `--tools <comma-separated-tools>`, `--verbose`, and "
    "`--worktree <worktree-name>`. "
    "The Hermes Agent instructions for this goal are passed verbatim as the `claude` positional prompt argument "
    "(quoted), with `--print` enabling non-interactive SDK mode; every other flag is mandatory and must be "
    "populated with a concrete, validated value before launch. "
...
# Flag inventory verified against the official Claude Code CLI reference at
# https://code.claude.com/docs/en/cli-reference (mapped via `firecrawl map
# --limit 5000 https://code.claude.com/docs`, scraped via `firecrawl scrape
# --format markdown --json https://code.claude.com/docs/en/cli-reference`).
# `--print` is the long form of the prompt-mode flag (`-p`); the prompt itself
# is the positional argument, not the flag's value. `--strict-mcp-config` is a
# boolean toggle that constrains MCP loading to whatever `--mcp-config` points
# at, so both flags are required when locking down MCP loading. `--p` and
# `--strict-mcp-config <file>` were the previous (invalid) shapes; the binary
# rejects them.
