...
`prerequisites` differs from `dependencies`: prerequisites are gating *conditions* (e.g. "Firecrawl auth verified", "Bun >= 1.1.30 installed"), while dependencies are concrete task ids that must be `completed` first. Both are required arrays; either may be empty `[]`.
...
| `Phase 2: GREEN`     | `GREEN`     | Smallest production code per RED suite, in dependency order. Each GREEN task references ≥1 specific RED task in its `dependencies` or `prerequisites`. |
...
- If Firecrawl was unavailable, set `validation_evidence.firecrawl.auth: unavailable`, leave `required_maps_present` empty (or `null`), and add an A* ANALYSIS task `prerequisites: ["Firecrawl auth verified"]` plus `validation_steps: ["firecrawl --status reports authenticated", ...]`.
...
- **GREEN → RED traceability**: every GREEN task references ≥1 RED task in `dependencies` ∪ `prerequisites` (transitive coverage through another GREEN dep is acceptable but flagged),
...
8. **Letting GREEN tasks edit production code before RED is committed.** Encode a `hard_gate` field on the RED phase block with explicit text refusing GREEN edits, seed an R00 (kickoff) task whose `blocking_tasks` lists every GREEN task. The `agent_runtime_protocol` section should reiterate the rule.
...
coding_agent_execution_contract:
  ...
  rule: >
    ...
    Hermes Agent instructions for this goal are passed verbatim as the `--p` payload;
    every other flag is mandatory and must be populated with a concrete, validated
  ...
  required_flags:
    - { name: "--p",                                  placeholder: "<instructions-from-hermes-agent>" }
    ...
    - { name: "--strict-mcp-config",                  placeholder: "<mcp-json-file>" }
  ...
  canonical_invocation: >
    claude --p <instructions-from-hermes-agent>
