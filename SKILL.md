...
See `references/revalidating-existing-files.md` for the full recipe and the list of substring-matched boilerplate paragraphs that must NOT be soft-wrapped across newlines. See `references/repository-context-exploration.md` for the deep workdir-introspection contract: what gets captured, how `--workdir` / `workdir=` plumbs through `prepare_goal_prompt`, `build_optimized_markdown`, and `write_task_list`, where the snapshot lands in the Markdown/YAML, and the contrarian-recheck rule for vanished key paths. See `references/research-tool-readiness.md` for handling requested Firecrawl/source-validation requirements without falsely claiming unavailable tooling was used. See `references/runtime-server-goal-edits.md` for adding concrete runtime/server contracts such as Mastra on Bun with Hono to an existing generated goal prompt. See `references/real-implementation-goal-prompts.md` when the user needs a generated goal that turns scaffold/template/descriptor-only work into real source-level implementation with exhaustive TDD gates. See `references/paradox-resolving-goal-prompts.md` when the user's request contains two apparently-contradictory constraints (e.g. "ship X from the start" + "X stays gitignored") that must both hold; the reference covers the deterministic-regeneration-from-in-tree-templates pattern, the Markdown additions, and the YAML `agent_runtime_protocol.paradox_invariant_check` protocol.
...
11. `## Scope`
12. `## Execution Plan`
13. `### Phase 0: BOOTSTRAP`
14. `### Phase 1: RED`
15. `### Phase 2: GREEN`
16. `### Phase 3: REFACTOR`
17. `## Software Development Constraints` when required
18. `## Software Engineering Core Principles` when the domain is software development
19. `## Acceptance Criteria`
20. `## Validation Commands`
21. `## Completion Definition`
22. `## Failure Conditions`
23. `## Final Output Requirements`
24. `## Coding Agent Execution Contract`
...
- [ ] The companion task list YAML carries a `coding_agent_execution_contract` top-level block whose `executor` is `claude`, whose `required_flags` enumerates each of the 18 required flags with a non-empty `placeholder`, whose `canonical_invocation` starts with `claude ` and contains every required flag, whose `worktree_directory` maps `--worktree` / `-w` to `.claude/worktrees/<worktree-name>` inside the active Hermes worktree, and whose `forbidden_alternatives` is a non-empty list.
...
35. **Re-typing the same source-claim assertions across runs instead of using `scripts/validate_source_claims.py`.** Every generation pass that names ≥1 opensrc repo and ≥1 cited file path SHOULD pre-flight those claims with the deterministic verifier. The script offers a `Verifier` class with `opensrc_repo(slug, key_paths)`, `file_substring(path, needle)`, `tcp_reachable(host)`, `tls_reachable(host)`, `firecrawl_usable()`, and `helper_present(path)`. Running it before writing the YAML catches fabricated paths and stale cached repo names in seconds. The script is dependency-free and runs from any backend. Import directly from any backend: `sys.path.insert(0, '<skill-dir>/scripts'); from validate_source_claims import Verifier; v = Verifier(); v.opensrc_repo(slug, key_paths)`. `opensrc_repo` expects the slug shape `<org>/<repo>` (for example `NousResearch/hermes-agent`), **not** `github.com/<org>/<repo>`; passing the host prefix makes it search `/root/.opensrc/repos/github.com/github.com/...` and falsely report `no main/ or master/ dir` even when the repo is cached. `opensrc_repo` returns a **bool**, not a dict — call `v.report()` for pass/fail counts, `v.failures` for the failed-check list, and `v.notes` for context like the L7-RST diagnostic. The script has docstring usage examples but no `__main__`, so don't expect `python validate_source_claims.py` to do anything useful — import the class. Two more shape gotchas that surfaced on macOS host runs (`/Users/<user>/.opensrc/...`):
    - **`opensrc_repo` hardcodes the cache root to `/root/.opensrc/repos/github.com/<slug>/<branch>`.** That's the Daytona/remote-backend path. On a Mac host the cache lives at `/Users/<user>/.opensrc/repos/github.com/<slug>/<branch>` and the verifier reports a false `no main/ or master/ dir` even when `opensrc list` shows the repo cached and `ls /Users/<user>/.opensrc/repos/github.com/<slug>/main/` succeeds. Don't rerun the verifier hoping the failure clears — read the source (`inspect.getsource(v.opensrc_repo)` shows the hardcoded `/root/.opensrc/repos/...` literal), confirm by hand with `ls /Users/$USER/.opensrc/repos/github.com/<slug>/main/`, and record the pass/fail honestly in `validation_evidence.opensrc.evidence[]` rather than papering over the false negative. The right durable fix is a follow-up patch to `validate_source_claims.py` that probes both `/root/.opensrc/...` and `pathlib.Path.home()/'.opensrc/repos/github.com/...'`; until that lands, treat a failed `opensrc_repo` check as inconclusive on macOS, not as a real claim failure.
    - **`firecrawl_usable()` is over-aggressive.** The check parses the `firecrawl --status` banner and treats any non-`Authenticated` line as failure, but Firecrawl 1.16.0 prints `.firecrawl: not found - no local cache` on a clean host with no `.firecrawl/` cache directory, and the verifier collapses that to `FAIL firecrawl --status authenticated :: ` (empty reason). The live `scripts/probe_firecrawl_reachability.py` is the canonical readiness check (it does a real `firecrawl map ... --json --pretty --limit 5000` and confirms `data.links` is populated); when the probe exits 0 with a positive `live_map_probe.url_count`, Firecrawl is usable regardless of what `validate_source_claims.firecrawl_usable()` reports. Run BOTH; trust the live probe; record both verdicts in `validation_evidence.firecrawl.evidence[]` with explicit phrasing like "live probe url_count=1260, source-claims verifier banner-parse over-aggressive (false negative on missing local cache)".
