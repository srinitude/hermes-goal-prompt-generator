...
Concrete, testable, user-facing criteria. Document assumptions, blockers, and trade-offs in the artifacts without asking the user for guidance during `/goal` execution.
...
Conditions that mean the goal is incomplete or invalid after all autonomous alternatives have been attempted, or when a non-bypassable legal, payment, credential, approval, or safety requirement prevents every remaining autonomous path.
