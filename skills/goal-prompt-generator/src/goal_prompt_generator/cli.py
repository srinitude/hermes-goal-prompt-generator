...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Which coding-agent CLI the generated contract should target. "
            "Valid choices: claude, codex, opencode, gemini, cursor, aider, pi, "
            "qwen, goose, amp, crush, hermes. "
            "Defaults to the first available installed CLI in catalog priority "
            "order, falling back to 'claude'. The environment variable "
            "GOAL_PROMPT_GENERATOR_EXECUTOR is honored when --executor is omitted."
        ),
    )
    return parser
